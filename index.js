...
// Этот модуль обрабатывает HTTP-запросы к серверу.
module.exports = createServer(async (req, res) => {
  if (req.url.substring(1, 6) === "image") {
    res.statusCode = 200;
    res.setHeader("Content-Type", "image/jpeg");
    //! CACHE КОНТРОЛЬ -  ЗАПРЕТИТЬ КЕШИРОВАНИЕ
  ...
  .on("listening", () => {
    // это событие, которое возникает, когда сервер начал прослушивать заданный порт
